  ...
  try:
    ...
    try:
      ...
      if os.path.exists(feedback_chroma_dir):
        logger.info(
            f"Removing existing feedback chroma directory: {feedback_chroma_dir}")
...
async def run_agent(request: AgentRequest):
  ...
  logger.info(
      f"Received request: input_text='{request.input_text}' tone='{request.tone}' platform='{request.platform}'")
  ...
  try:
    ...
    try:
      ...
      return JSONResponse(
          status_code=500,
          content={
              "error": "Failed to initialize resources. Please try again later."}
      )
  ...
  try:
    ...
    try:
      ...
      retrieved_docs = vectorstore.similarity_search(
          query, k=2)  # Retrieve top 2 relevant documents
      ...
      logger.error(
          f"Error in similarity search: {str(search_error)}", exc_info=True)
...
def feedback_retriever_node(state: RewriteState) -> RewriteState:
  ...
  try:
    ...
    if feedback_vectorstore is None:
      _, feedback_vectorstore = get_resources()

    # Perform similarity search
    try:
      logger.info(f"Performing similarity search for query: {query}")
      retrieved_docs = feedback_vectorstore.similarity_search(
          query, k=5)  # Retrieve top 5 relevant documents
      ...
      if not user_feedback:
        ...
        logger.info(
            f"Retrieved {len(retrieved_docs)} relevant documents and processed feedback.")
...
async def receive_feedback(feedback: FeedbackInput):
  ...
  try:
    ...
    feedback_text = f"Feedback for {feedback_dict['platform']} ad with {feedback_dict['tone']} tone and rating {feedback_dict['rating']}: {feedback_dict['feedback']}"

    # Add feedback to vectorstore
    feedback_vectorstore.add_documents([Document(page_content=feedback_text)])

    # rating = feedback.get("rating")
    feedback_store.append(feedback)
    feedback_analyzer.add_feedback(feedback)

    # Log the feedback for analysis
    rating = feedback_dict.get("rating")
    logger.info(
        f"New feedback received - Rating: {rating}, Issues: {feedback_analyzer.common_issues}")
...
async def get_feedback_insights():
  ...
  try:
    ...
    raise HTTPException(
        status_code=500, detail="Error retrieving feedback insights")
